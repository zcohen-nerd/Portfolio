...
<Figure
  src="/assets/images/projects/stlink-v3mods/spark-board-perspective.webp"
  alt="SPARK board perspective render"
  width={1046}
  height={697}
  maxWidth={800}
  maxHeight="none"
...
<ProjectAtAGlance
  ...
  role="Sole designer — schematic, 4-layer PCB, protection and power architecture, and the fabrication package"
  links={[
    {
      href: 'https://github.com/zcohen-nerd/SPARK',
      label: 'Source on GitHub — schematic, PCB, fabrication outputs, BOM, validation plan',
    },
  ]}>
...
- 22 Ω series resistance and ESD (electrostatic-discharge) protection arrays on every external-facing signal line
- Schematic, 4-layer PCB, ODB++ fabrication package, and BOM in the repository; bench-validation plan defined, not yet executed
...
In the repository: the **schematic** and **PCB layout** sources, the design archive, an **ODB++ fabrication output tree**, the master **BOM**, and CAD board renders. The fabrication output is **ODB++ only** — RS-274X Gerbers and Excellon drill files are not currently exported. The layout uses heavy ground flooding with via stitching, 12-mil typical routing, and an ENIG finish with ink-plugged vias.

**Electronics schematic**

<Figure
  src="/assets/images/projects/stlink-v3mods/Schematic.png"
  alt="SPARK V0.4 electronics schematic"
  width={1024}
  height={663}
/>
...
<Figure
  src="/assets/images/projects/stlink-v3mods/PCB.png"
  alt="SPARK V0.4 PCB layout"
  width={375}
  height={375}
/>

**No fabrication, assembly, or measurement records are published in the repository.** The [SPARK repository](https://github.com/zcohen-nerd/SPARK#status) tracks maturity per stage: *Designed* and *Fabrication package produced* are done; *Fabricated*, *Assembled*, *Electrically verified*, and *Bench-validated* are not. Fabrication receipts, board photos, assembly notes, and measurement records will be added there — and reflected here — as they exist.

## Testing & Verification

A [bench-validation plan](https://github.com/zcohen-nerd/SPARK/blob/main/validation/test_plan.md) and a [first-article bring-up procedure](https://github.com/zcohen-nerd/SPARK/blob/main/validation/bring_up.md) are defined in the repository. **They have not been executed and no measurement records are published.** Planned V1 hardware goals already identified from design review include a VTREF overvoltage-protection path and an optional split CAN termination network.

When bring-up happens, this section will record what was validated and what needed rework:
...
A real measurement or a documented mistake will carry more weight here than another render; failures and rework will be recorded, not hidden.

## Current Status

**Prototype.** Schematic, 4-layer PCB layout, ODB++ fabrication package, and BOM are complete in the repository. Nothing has been fabricated, assembled, or electrically verified in the repository record. The next milestone is a first-article board and executing the bring-up procedure against it.
  ...
  items={[
    {
      ...
      reason:
        'A 20-vessel fleet whose four custom boards were brought up board-by-board — the real work this interface board is meant to de-risk.',
    },
    {
      href: 'https://zcohen-nerd.github.io/connector-engineering-field-guide/',
      title: 'Connector Engineering Field Guide',
      reason:
        'SPARK exposes SWD/JTAG, UART, SPI, I²C, and CAN FD through standard connectors; the guide covers choosing and specifying them.',
    },
    {
      href: 'https://github.com/zcohen-nerd/PinmapGen',
      title: 'PinmapGen',
      reason:
        'Generates firmware pin-maps from Fusion Electronics schematics — the front half of the embedded bring-up workflow SPARK sits in.',
    },
  ]}
/>
