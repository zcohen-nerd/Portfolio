...
In the repository: the **schematic** and **PCB layout** sources, the design archive, an **ODB++ fabrication output tree**, the master **BOM**, and board renders. (Gerber archives are not published yet — the board is still in beta bring-up.) The layout uses heavy ground flooding with via stitching, 12-mil typical routing, and an ENIG finish with ink-plugged vias.
