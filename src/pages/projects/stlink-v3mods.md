...
- 4-layer controlled-impedance PCB (JLC04161H-7628), 88.9 mm × 88.9 mm enclosure-driven envelope
...
- 22 Ω series impedance and ESD arrays on every external-facing signal line
...
A [bench-validation plan](https://github.com/zcohen-nerd/SPARK/blob/main/validation/test_plan.md) is defined in the repository, but **execution remains in progress** — no completed measurement records are published yet. Planned V1 hardware goals already identified from design review include a VTREF overvoltage-protection path and an optional split CAN termination network.

## Current Status

**Prototype.** Requirements and validation are active; schematic and layout are complete; the next milestones are physical fabrication, assembly, and executing the bench-validation plan.

## Lessons Learned

- Interface hardware earns its complexity: every protective element (eFuse, default-off rails, series impedance) exists because of a specific failure mode seen in real bring-up work.
